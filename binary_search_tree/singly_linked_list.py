...
class LinkedList:
    ...
    def add_to_tail(self, value):
        # the following 3 steps assume the tail is already
        # referring to a Node
        # #1 create the Node from the value
        new_node = Node(value)

        # check for a head & tail
        if self.head is None and self.tail is None:
            # in case of one-element linked list,
            # head and tail should refer to the new node
            self.head = new_node
    ...
    def remove_tail(self):
        ...
        val = self.tail.get_value()
        # move self.tail to the second to last Node right before the tail
        self.tail = current
        # remove link to existing tail
